...
namespace FrodLib.Commands
{
    ...
    public class AsyncRelayCommand<T> : ICommand, IDisposable
    {
        ...
        protected Func<T, Task> _execute = null;
        ...
        /// <param name="execute">The execution logic.</param>
        /// <param name="canExecute">The execution status logic.</param>
        public AsyncRelayCommand(Func<T, Task> execute, Predicate<T> canExecute, IInvoker invoker)
        {
            if (execute == null)
        ...
        public AsyncRelayCommand<T> ListenForNotificationFrom<TObservedType>(TObservedType viewModel) where TObservedType : INotifyPropertyChanged
        {
            if (viewModel == null) return this;
