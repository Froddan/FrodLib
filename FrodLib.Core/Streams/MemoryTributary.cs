...
namespace FrodLib.Streams
{
    ...
    public class MemoryTributary : Stream       /* http://msdn.microsoft.com/en-us/library/system.io.stream.aspx */
    {
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            if (lcount < 0)
            {
                throw new ArgumentOutOfRangeException("count", StringResources.BytesCopyCannotBeNegative);
            }
            ...
            if (buffer == null)
            {
                throw new ArgumentNullException("buffer", StringResources.BufferCannotBeNull);
            }
            if (offset < 0)
            {
                throw new ArgumentOutOfRangeException("offset", StringResources.DestinationOffsetCannotBeNegative);
            }
        ...
        public static implicit operator MemoryTributary(MemoryStream stream)
        {
            MemoryTributary memoryTribunary = new MemoryTributary();
            memoryTribunary.ReadFrom(stream, stream.Length);
            return memoryTribunary;
        }

        public static explicit operator MemoryStream(MemoryTributary tribunary)
        {
            MemoryStream stream = new MemoryStream();
            tribunary.WriteTo(stream);
            return stream;
        }
    }
}
